...
public class StatementParser {
    ...
    public Type parseType(VersyParser p, bool getNumbers = false) {
        ...
        while (p.getCurrentTokenType() == Tokens.LSBRACKET) {
            ...
            firstTime = false;
            int? size = null;
            if (p.getCurrentTokenType() == Tokens.NUMBER && getNumbers) size = (int) (double) p.expect(Tokens.NUMBER).value;
            else if (p.getCurrentTokenType() == Tokens.NUMBER) p.advance();
